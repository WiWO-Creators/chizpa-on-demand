...
export function PaymentSuccess({ sessionId }: { sessionId: string }) {
  ...
  return <main className="success-page">
    <div className="success-card">
      ...
      {state === "ready" && <><CheckCircle2 size={44} /><p className="section-kicker">Pago listo</p><h1>Tu Chizpa está encendida.</h1><p>Pedido <strong>{order?.human_code}</strong>. Chispita ya convirtió tus respuestas en instrucciones comprensibles.</p><Link className="button button--purple" href={`/track?session_id=${encodeURIComponent(sessionId)}`}>Seguir mi proyecto <ArrowRight size={18} /></Link></>}
